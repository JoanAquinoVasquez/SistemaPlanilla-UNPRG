...
function Login() {
  ...
  const handleGoogleLoginSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      if (error.response) {
        ...
        setErrorMessage(error.response.data.error); // Establece el mensaje de error
      } else {
        setErrorMessage(
          "Error al autenticar con el backend. Intenta nuevamente."
        );
      }
  ...
  const handleGoogleLoginError = () => {
    ...
    setErrorMessage("Error al iniciar sesión con Google."); // Maneja el error aquí si es necesario
  };
