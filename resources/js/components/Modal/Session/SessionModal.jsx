  ...
  return (
    <Modal backdrop="opaque" isOpen={isOpen} onOpenChange={onClose} placement="top-center" size="lg">
      <ModalContent>
        ...
        <ModalFooter>
          {type === "expiracion" && (
            <Button color="danger" variant="flat" onPress={() => { onLogout(); onClose(); }}>Cerrar Sesión</Button>
          )}
          <Button color="primary" onPress={() => { onKeepSession(); onClose(); }}>Mantener Sesión</Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
  );
